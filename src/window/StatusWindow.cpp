...
void StatusWindow::onRender(TextBuffer& buffer)
{
    ...
    renderString(buffer, "Weight: " + std::to_string(m_player->inventoryWeight()) + " / " + std::to_string(m_player->inventoryWeightLimit()), Viewport{{2, 4}, {m_viewport.m_size.m_width - 4, 1}}, TextAlign::Center);
    renderString(buffer, "Money: " + std::to_string(m_player->money()), Viewport{{2, 5}, {m_viewport.m_size.m_width - 4, 1}}, TextAlign::Center);
